...
//static a2b_PalEcb* pPalEcb; //ESTEC_A2B_STACK_PORTING
A2B_PAL_L3_DATA
...
{
    //a2b_Char* envValue; //ESTEC_A2B_STACK_PORTING
    ...
    if ( A2B_NULL != pal )
    {
		...
		if ( A2B_NULL != ecb )
		{
			...
			ecb->baseEcb.i2cBusSpeed   = A2B_I2C_BUS_SPEED_400KHZ; //KMS241206_1 : A2B I2C speed is fixed 400K in A2B and MCU both sides.
...
{
	a2b_HResult nReturnValue = NULL;
    nReturnValue = adi_a2b_TwiWriteRead(hnd, addr, nWrite, (a2b_UInt8*)wBuf,
    		                                  nRead, rBuf );
