...
a2b_UInt32 adi_a2b_EnablePinInterrupt(a2b_UInt8 nGPIONum , void* pUserCallBack, a2b_UInt32 CallBackParam)
{
	...
	eResult = adi_gpio_SetDirection(ADI_GPIO_PORT_H, ADI_GPIO_PIN_4, ADI_GPIO_DIRECTION_INPUT);

	/* set edge sense mode (PORT H is connected to Pin Interrupt 3) */
	eResult = adi_gpio_SetInputEdgeSense(ADI_GPIO_PORT_H, ADI_GPIO_PIN_4, ADI_GPIO_SENSE_RISING_EDGE);
	eResult = adi_gpio_EnableInterruptMask(ADI_GPIO_PORT_H,ADI_GPIO_PIN_4, ADI_GPIO_MASK_A, TRUE );
	eResult = adi_gpio_RegisterCallback(ADI_GPIO_PORT_H, ADI_GPIO_PIN_4, (ADI_GPIO_CALLBACK)&adi_a2b_PinInterruptHandler, (void*)pUserCallBack);
