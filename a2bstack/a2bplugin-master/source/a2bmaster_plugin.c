		...
		a2b_UInt8 nDataSize, a2b_Int16 nSlvNodeAddr, a2b_UInt8 nMBoxNo)
{
	...
	if(A2B_FAILED(status))
	{
		...
		cprintf("Exit: %s execute(): Mailbox Write Failed",A2B_MPLUGIN_PLUGIN_NAME);
#else
    ...
    )
{
	...
	{
#ifdef ESTEC_A2B_STACK_PORTING_DEBUG_ENABLE
		cprintf("a2b_onMboxTxResponseTimeout: failed to tick Communication channel Node Address: 0x%lX", NodeAddr);
#else
...
static void a2b_CommChCallBk(void* pCbParam, a2b_CommChMsg *pMsg, A2B_COMMCH_EVENT eEventType, a2b_Int8 nNodeAddr)
{
	...
	else
	{
		...
		else
		{
			...
			{
#ifdef ESTEC_A2B_STACK_PORTING_DEBUG_ENABLE
				cprintf("Exit: %s: Communication channel Event Processing by Plugin failed", A2B_MPLUGIN_PLUGIN_NAME);
#else
    ...
    )
{
    ...
    switch ( cmd )
    {
        ...
        case A2B_MSGREQ_NET_BERT_START:
        	...
        	adi_a2b_BertIntiation((a2b_UInt8 *)pBertMsg->req.pBertConfigBuf , plugin);
        	break;
