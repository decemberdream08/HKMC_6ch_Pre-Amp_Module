    ...
    )
{
    ...
    else if ( A2B_NULL == ctx->stk->pluginList[A2B_NODEADDR_MASTER+1] )
    {
        ...
        if ( !A2B_SUCCEEDED(ret) )
        {
			...
			cprintf("\n\ra2b_msgRtrInitMasterPlugin(0x%p): 0x%lX, Failed to init master plugin. \r\n",ctx, &ret);
#else
    ...
    )
{
    ...
    else
    {
...
#ifdef ESTEC_A2B_STACK_PORTING_DEBUG_ENABLE
		cprintf("\n\rEnter: a2b_msgRtrSendRequestToMailbox(0x%p)", ctx);
#else
        A2B_TRACE1((ctx, (A2B_TRC_DOM_MSGRTR | A2B_TRC_LVL_TRACE1),
        ...
        if ( A2B_SUCCEEDED(ret) )
        {
            ...
            {
#ifdef ESTEC_A2B_STACK_PORTING_DEBUG_ENABLE
				cprintf("\n\rExit: a2b_msgRtrSendRequestToMailbox(0x%p): 0x%lX, Failed to init master.",
                            ctx, &ret);
#else            
                A2B_TRACE2((ctx, (A2B_TRC_DOM_MSGRTR | A2B_TRC_LVL_TRACE1),
            ...
            for ( idx = 0u; idx < (a2b_UInt16)A2B_ARRAY_SIZE(ctx->stk->pluginList); idx++ )
            {
                ...
                if ( pluginCtx && jobQ )
                {
                    ...
                    {
#ifdef ESTEC_A2B_STACK_PORTING_DEBUG_ENABLE
						cprintf("\n\ra2b_msgRtrSendRequestToMailbox(0x%p, addr: %hd, cmd: %d): Failed",
                                    pluginCtx, 
                                    &msg->destNodeAddr,
                                    &msg->cmd);
#else                    
                    ...
                    else
                    {
                        ...
                        a2b_msgRef(msg);
#ifdef ESTEC_A2B_STACK_PORTING_DEBUG_ENABLE
						cprintf("\n\ra2b_msgRtrSendRequestToMailbox(0x%p, addr: %hd, cmd: %d)",
                            pluginCtx, 
                            &msg->destNodeAddr,
                            &msg->cmd);

						cprintf("\n\ra2b_msgRtrSendRequest (m: 0x%p, cmd: %ld, ud: 0x%p)",
                            msg, &msg->cmd, msg->userData);
#else						
                        A2B_TRACE3((ctx, 
                            (A2B_TRC_DOM_MSGRTR | A2B_TRC_LVL_TRACE1),
...
#ifdef ESTEC_A2B_STACK_PORTING_DEBUG_ENABLE
		cprintf("\n\rExit: a2b_msgRtrSendRequestToMailbox(0x%p): 0x%lX", 
                    ctx, &ret);
#else
