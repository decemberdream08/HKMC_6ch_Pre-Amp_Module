...
static void adi_a2b_CheckforAutoConfig(bdd_Node *pNode, a2b_Bool bAutoConfig);
#endif
		...
		bdd_Network  *bdd_Graph, a2b_UInt8 nBusIndex )
{
    ...
    (void)memset(bdd_Graph, 0, sizeof(bdd_Network));
    ...
    )
{
	...
	for(nIndex1 = 0u ; nIndex1 < (a2b_UInt8)pMasterSlaveChain->nNumSlaveNode; nIndex1++)
	{
		...
		if(A2B_IS_AD242X_CHIP(pSlvNode->nodeDescr.vendor, pSlvNode->nodeDescr.product) ||
		   pSlvNode->nodeDescr.bIsAnalyzer)
		{
...
#ifdef ESTEC_A2B_STACK_PORTING_DEBUG_ENABLE
	cputs("\n\radi_a2b_ParseSlaveNCD()");
#endif

	pSlvNode->ignEeprom = (pSlvCfg->bEnableAutoConfig)^ 0x1;/*AUTO-CONFIG*/
	pSlvNode->nodeSetting.has_bLocalPwrd = pSlvCfg->bLocalPower;
...
)
{
	...
	pMstrNode->pinIoRegs.clk2cfg |= (pMstCfg->sClkOutSettings.bClk2Div << (a2b_UInt8)A2B_BITP_CLKOUT2_CLK1DIV) |\
...
*  if so sets the HAS status in the bdd_node structure as 'A2B_FALSE' so that stack
...
static void adi_a2b_CheckforDefault(bdd_Node *pNode)
{
	...
	A2B_UPDATE_HAS(pNode->mbox,mbox1ctl, 2);
...
{
#ifdef ESTEC_A2B_STACK_PORTING_DEBUG_ENABLE
	cputs("\n\radi_a2b_SetforAllReg_242x_master()");
#endif
...
{
#ifdef ESTEC_A2B_STACK_PORTING_DEBUG_ENABLE
	cputs("\n\radi_a2b_SetforAllReg_242x_slave()");
#endif
...
{
#ifdef ESTEC_A2B_STACK_PORTING_DEBUG_ENABLE
	cputs("\n\radi_a2b_SetforAllReg_241x_slave()");
#endif
...
{
#ifdef ESTEC_A2B_STACK_PORTING_DEBUG_ENABLE
	cputs("\n\radi_a2b_SetforAllReg_241x_master()");
#endif
